...
class ComercializationComplement(ComplementBuilder):
    ...
    def __validate_almacenamiento(self, alm: dict) -> None:
        ...
        if alm_dist_terminal is None:
            raise KeyError("Error: clave 'TerminalAlmYDist' no encontrada")

        if not 5 <= len(alm_dist_terminal) <= 250:
            raise LongitudError(
                f"Error: clave 'PermisoAlmYDist' con valor {alm_dist_alm} no tiene la longitud min 5 o max 250."
            )
        if not re.match(PERMISSION_ALM_DIST_REGEX, alm_dist_alm):
            raise RegexError(
                f"Error: clave 'PermisoAlmYDist' con valor {alm_dist_alm} no cumple con el patrón {PERMISSION_ALM_DIST_REGEX}"
            )
    ...
    def __validate_transporte(self, transp: dict) -> None:
        ...
        if transp_cap_fee and not 0 <= transp_cap_fee <= 1000000000000:
            raise ValorMinMaxError(
                f"Error: clave 'CargoPorCapacidadTrans' con valor {transp_cap_fee} no se encuentra en el rango min 0 o max 1000000000000."
            )
